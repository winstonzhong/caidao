...
# def 从字符串提取时间并转为秒(txt: str) -> int:
#     """
#     从字符串中提取"回复评论 回复"前的时间信息并转换为秒数，非当天时间统一返回86400秒。

#     转换规则：
#     1. x分钟前 → x * 60 秒
#     2. x小时前 → x * 3600 秒
#     3. 昨天（含时分）、周一至周日等非当天时间 → -1 秒
#     4. 未匹配到有效时间 → -1 秒

#     Examples:
#     >>> 从字符串提取时间并转为秒("如意好物的头像 如意好物 评论了你: 漂亮的美女[赞] 21分钟前 回复评论 回复,按钮 赞 赞,按钮")
#     1260
#     >>> 从字符串提取时间并转为秒("风火轮的头像 风火轮 回复: [来看我] 41分钟前 回复评论 回复,按钮 赞 赞,按钮")
#     2460
#     >>> 从字符串提取时间并转为秒("丽Li🍀的头像 丽Li🍀 作者 回复: [比心] 1小时前 回复评论 回复,按钮 赞 赞,按钮")
#     3600
#     >>> 从字符串提取时间并转为秒("恭喜发财的头像 恭喜发财 互相关注 回复: 厚德载物 一带到[比心] 加油 4小时前 回复评论 回复,按钮 赞 赞,按钮")
#     14400
#     >>> 从字符串提取时间并转为秒("坤姐儿的头像 坤姐儿 互相关注 回复: 是啊，总是失去了才会珍惜 昨天 21:35 回复评论 回复,按钮 赞 赞,按钮")
#     -1
#     >>> 从字符串提取时间并转为秒("听不见的小白的头像 回复: 1 昨天 00:15 回复评论 回复,按钮 赞 赞,按钮")
#     -1
#     >>> 从字符串提取时间并转为秒("Qc2706的头像 Qc2706 作者 回复: 在老地方等你[比心] 周三 回复评论 回复,按钮 赞 赞,按钮")
#     -1
#     >>> 从字符串提取时间并转为秒("汤圆柠檬味的头像 汤圆柠檬味 回复@拼搏哥: 我今天拆退货发现客户买两件少退了一件，马上找...  周三 回复评论 回复,按钮 赞 赞,按钮")
#     -1
#     >>> 从字符串提取时间并转为秒("小梁的头像 回复评论 回复,按钮 赞 赞,按钮")
#     -1
#     >>> 从字符串提取时间并转为秒("缘惜的头像 缘惜 评论了你: 回复@小蜜蜂: 1 昨天 21:21 回复评论 回复,按钮 赞 赞,按钮")
#     -1
#     >>> 从字符串提取时间并转为秒("小蜜蜂的头像 回复: 1 回复评论 回复,按钮 赞 赞,按钮")
#     -1
#     """
#     # 正则模式：匹配"回复评论 回复"前的时间部分
#     # 覆盖：分钟前/小时前/昨天(含时分)/中文星期
#     pattern = r"((?:\d+分钟前|\d+小时前|昨天(?:\s+\d{2}:\d{2})?|周一|周二|周三|周四|周五|周六|周日))\s+回复评论 回复"
...
def 从字符串提取时间并转为秒(txt: str) -> tuple[int, str]:
    ...
    1. x分钟前 → x * 60 秒
    2. x小时前 → x * 3600 秒
    3. 昨天（含时分）、周一至周日等非当天时间 → 86400 秒
    4. 未匹配到有效时间 → -1 秒

    Examples:
    >>> r = 从字符串提取时间并转为秒("如意好物的头像 如意好物 评论了你: 漂亮的美女[赞] 21分钟前 回复评论 回复,按钮 赞 赞,按钮")
    >>> r[0]
    1260
    >>> r[1]
    '如意好物的头像 如意好物 评论了你: 漂亮的美女[赞]  回复评论 回复,按钮 赞 赞,按钮'
    >>> r = 从字符串提取时间并转为秒("风火轮的头像 风火轮 回复: [来看我] 41分钟前 回复评论 回复,按钮 赞 赞,按钮")
    >>> r[0]
    2460
    >>> r[1]
    '风火轮的头像 风火轮 回复: [来看我]  回复评论 回复,按钮 赞 赞,按钮'
    >>> r = 从字符串提取时间并转为秒("丽Li🍀的头像 丽Li🍀 作者 回复: [比心] 1小时前 回复评论 回复,按钮 赞 赞,按钮")
    >>> r[0]
    3600
    >>> r[1]
    '丽Li🍀的头像 丽Li🍀 作者 回复: [比心]  回复评论 回复,按钮 赞 赞,按钮'
    >>> r = 从字符串提取时间并转为秒("恭喜发财的头像 恭喜发财 互相关注 回复: 厚德载物 一带到[比心] 加油 4小时前 回复评论 回复,按钮 赞 赞,按钮")
    >>> r[0]
    14400
    >>> r[1]
    '恭喜发财的头像 恭喜发财 互相关注 回复: 厚德载物 一带到[比心] 加油  回复评论 回复,按钮 赞 赞,按钮'
    >>> r = 从字符串提取时间并转为秒("坤姐儿的头像 坤姐儿 互相关注 回复: 是啊，总是失去了才会珍惜 昨天 21:35 回复评论 回复,按钮 赞 赞,按钮")
    >>> r[0]
    86400
    >>> r[1]
    '坤姐儿的头像 坤姐儿 互相关注 回复: 是啊，总是失去了才会珍惜  回复评论 回复,按钮 赞 赞,按钮'
    >>> r = 从字符串提取时间并转为秒("听不见的小白的头像 回复: 1 昨天 00:15 回复评论 回复,按钮 赞 赞,按钮")
    >>> r[0]
    86400
    >>> r[1]
    '听不见的小白的头像 回复: 1  回复评论 回复,按钮 赞 赞,按钮'
    >>> r = 从字符串提取时间并转为秒("Qc2706的头像 Qc2706 作者 回复: 在老地方等你[比心] 周三 回复评论 回复,按钮 赞 赞,按钮")
    >>> r[0]
    86400
    >>> r[1]
    'Qc2706的头像 Qc2706 作者 回复: 在老地方等你[比心] 周三 回复评论 回复,按钮 赞 赞,按钮'
    >>> r = 从字符串提取时间并转为秒("汤圆柠檬味的头像 汤圆柠檬味 回复@拼搏哥: 我今天拆退货发现客户买两件少退了一件，马上找...  周三 回复评论 回复,按钮 赞 赞,按钮")
    >>> r[0]
    86400
    >>> r[1]
    '汤圆柠檬味的头像 汤圆柠檬味 回复@拼搏哥: 我今天拆退货发现客户买两件少退了一件，马上找...  周三 回复评论 回复,按钮 赞 赞,按钮'
    >>> r = 从字符串提取时间并转为秒("小梁的头像 回复评论 回复,按钮 赞 赞,按钮")
    >>> r[0]
    86400
    >>> r[1]
    '小梁的头像 回复评论 回复,按钮 赞 赞,按钮'
    >>> r = 从字符串提取时间并转为秒("缘惜的头像 缘惜 评论了你: 回复@小蜜蜂: 1 昨天 21:21 回复评论 回复,按钮 赞 赞,按钮")
    >>> r[0]
    86400
    >>> r[1]
    '缘惜的头像 缘惜 评论了你: 回复@小蜜蜂: 1  回复评论 回复,按钮 赞 赞,按钮'
    >>> r = 从字符串提取时间并转为秒("小蜜蜂的头像 回复: 1 回复评论 回复,按钮 赞 赞,按钮")
    >>> r[0]
    86400
    >>> r[1]
    '小蜜蜂的头像 回复: 1 回复评论 回复,按钮 赞 赞,按钮'
    >>> r = 从字符串提取时间并转为秒("🎊招财包🇨🇳,赞了你的评论 7分钟前，,按钮")
    >>> r[0]
    420
    >>> r[1]
    '🎊招财包🇨🇳,赞了你的评论 ，,按钮'
    >>> r = 从字符串提取时间并转为秒("老玩童,赞了你的评论 5小时前，,按钮")
    >>> r[0]
    18000
    >>> r[1]
    '老玩童,赞了你的评论 ，,按钮'
