import pandas as pd
import re
import tool_time
import numpy as np
from collections import Counter


def 网友评论解析(line: str) -> list[dict]:
    """
    解析单行网友评论文本，提取关键信息并返回字典列表

    参数:
        line: 单行评论文本，包含网友信息、行为、时间、bounds box等

    返回:
        list[dict]: 包含以下字段的字典列表：
            - 网友名称: str，网友的昵称（无则为空字符串）
            - 评论内容: str，评论/回复的具体内容（无则为空字符串）
            - 是否需回复: bool，是否包含"回复评论 回复,按钮"特征
            - left: int，bounds box左坐标
            - top: int，bounds box上坐标
            - right: int，bounds box右坐标
            - bottom: int，bounds box下坐标

    Doctest测试用例:
    >>> # 测试1：仅包含bounds box的空行
    >>> 网友评论解析(" (0, 2167, 1080, 2236)")
    {'网友名称': '', '评论内容': '', '是否需回复': False, 'left': 0, 'top': 2167, 'right': 1080, 'bottom': 2236}
    >>> # 测试2：赞了你的评论，无回复按钮
    >>> line2 = "魏晓樱Queen的头像 魏晓樱Queen 赞了你的评论 1分钟前 (0, 204, 1080, 447)"
    >>> 网友评论解析(line2)
    {'网友名称': '魏晓樱Queen', '评论内容': '', '是否需回复': False, 'left': 0, 'top': 204, 'right': 1080, 'bottom': 447}
    >>> # 测试3：评论了你，包含回复按钮（需回复）
    >>> line3 = "唯有欢喜的头像 唯有欢喜 粉丝 评论了你: 新年快乐 1分钟前 回复评论 回复,按钮 赞 赞,按钮 (0, 447, 1080, 709)"
    >>> res3 = 网友评论解析(line3)
    >>> res3['网友名称'], res3['评论内容'], res3['是否需回复']
    ('唯有欢喜', '新年快乐', True)
    >>> res3['left'], res3['top'], res3['right'], res3['bottom']
    (0, 447, 1080, 709)
    >>> # 测试4：带表情的评论内容
    >>> line4 = "花木蓝的头像 花木蓝 评论了你: 我也不知道有啥用[大笑][大笑] 23分钟前 回复评论 回复,按钮 赞 赞,按钮 (0, 622, 1080, 1136)"
    >>> d = 网友评论解析(line4)
    >>> d['评论内容']
    '我也不知道有啥用[大笑][大笑]'
    >>> d["是否需回复"]
    True
    >>> # 测试5：作者回复的场景
    >>> line5 = "花木蓝的头像 花木蓝 作者 回复: 是的，人间烟火气息[大笑] 32分钟前 回复评论 回复,按钮 赞 赞,按钮 (0, 1379, 1080, 1711)"
    >>> res5 = 网友评论解析(line5)
    >>> res5['网友名称'], res5['评论内容']
    ('花木蓝', '是的，人间烟火气息[大笑]')
    >>> res5['是否需回复']
    False
    >>> # 测试6：名称含特殊字符的场景
    >>> line6 = "农村阿娟带娃记（新人起步）的头像 农村阿娟带娃记（新人起步） 赞了你的评论 7分钟前 (0, 1195, 1080, 1438)"
    >>> 网友评论解析(line6)['网友名称']
    '农村阿娟带娃记（新人起步）'
    >>> # 测试7：赞了你的图文场景
    >>> line7 = "花木蓝的头像 花木蓝 赞了你的图文 23分钟前 (0, 1136, 1080, 1379)"
    >>> 网友评论解析(line7)['评论内容']
    ''
    >>> 网友评论解析(line7)['是否需回复']
    False
    """
    # 初始化返回结果

    # 正则匹配行末尾的bounds box（捕获left/top/right/bottom）
    bounds_pattern = re.compile(r"\((\d+),\s*(\d+),\s*(\d+),\s*(\d+)\)$")
    bounds_match = bounds_pattern.search(line)

    # 提取bounds box坐标（默认0，防止匹配失败）
    left = int(bounds_match.group(1)) if bounds_match else 0
    top = int(bounds_match.group(2)) if bounds_match else 0
    right = int(bounds_match.group(3)) if bounds_match else 0
    bottom = int(bounds_match.group(4)) if bounds_match else 0

    # 去掉bounds box后的纯文本内容（用于提取其他信息）
    text_without_bounds = bounds_pattern.sub("", line).strip()

    # 1. 提取网友名称：匹配"的头像"后的名称（直到遇到粉丝/作者/赞了/评论了你/回复:等关键词）
    # name_pattern = re.compile(
    #     r"的头像\s+([^(\s+粉丝|\s+作者|\s+赞了|\s+评论了你|\s+回复:)]+)"
    # )
    name_pattern = re.compile(r"(.+?)的头像")
    name_match = name_pattern.search(text_without_bounds)
    网友名称 = name_match.group(1).strip() if name_match else ""

    # 2. 提取评论内容：优先匹配"评论了你:"，其次匹配"回复:"，无则为空
    content_pattern1 = re.compile(r"评论了你:\s*(.+?)\s+\d+分钟前")  # 评论你的场景
    content_pattern2 = re.compile(r"回复:\s*(.+?)\s+\d+分钟前")  # 作者回复的场景
    content_match1 = content_pattern1.search(text_without_bounds)
    content_match2 = content_pattern2.search(text_without_bounds)
    if content_match1:
        评论内容 = content_match1.group(1).strip()
    elif content_match2:
        评论内容 = content_match2.group(1).strip()
    else:
        评论内容 = ""

    # 3. 判断是否需要回复：精确匹配"回复评论 回复,按钮"
    是否需回复 = (
        "回复评论 回复,按钮" in text_without_bounds and content_match1 is not None
    )

    # 构建结果字典并添加到列表
    return {
        "网友名称": 网友名称,
        "评论内容": 评论内容,
        "是否需回复": 是否需回复,
        "left": left,
        "top": top,
        "right": right,
        "bottom": bottom,
    }


def 网友评论提取(job, results: list):
    data = []
    for e in results:
        line = f"{job.元素转字符串(e)}, {e.bounds}"
        # print(line)
        d = 网友评论解析(line)
        d["e"] = e
        d["秒数"] = tool_time.从字符串提取时间并转为秒(line, np.nan)[0]
        d["是否需回复"] = (
            False
            if not d.get("是否需回复")
            else d.get("网友名称") not in job.数据.定长队列
        )
        data.append(d)
    return pd.DataFrame(data)


def 提取链接(txt: str) -> list:
    """
    从输入文本中提取所有HTTP/HTTPS协议的链接

    Doctest单元测试用例：
    >>> 提取链接('1.76 复制打开抖音，看看【河马包包的菜园的作品】昭昭如愿，岁岁安澜# 热门 # 抖音美食推荐官 #... https://v.douyin.com/XrmRMhEVlbk/ 11/20 z@t.eb lcN:/ ')
    ['https://v.douyin.com/XrmRMhEVlbk/']
    >>> 提取链接('无链接的纯文本内容')
    []
    >>> 提取链接('多个链接测试：https://www.baidu.com 测试内容 https://www.google.com/index.html 结尾')
    ['https://www.baidu.com', 'https://www.google.com/index.html']
    >>> 提取链接('http开头的链接：http://example.com/123 abc')
    ['http://example.com/123']
    """
    # 正则表达式模式：匹配http/https开头，直到遇到空白字符为止的所有字符
    # https? 匹配http或https；:// 匹配协议后的固定分隔符；[^\s]+ 匹配任意非空白字符（直到空格结束）
    link_pattern = r"https?://[^\s]+"

    # 查找所有匹配的链接，返回列表
    matched_links = re.findall(link_pattern, txt)

    return matched_links


def 是否无内容(txt: str):
    """
    是否无内容 的 Docstring

    :param txt: 说明
    :type txt: str
    >>> 是否无内容('由于未提供具体短视频内容，无法生成精准评论~')
    True
    >>> 是否无内容(None)
    True
    >>> 是否无内容('内容')
    False
    """
    return re.search("(无法生成|链接)", txt) is not None if txt else True


def 是否为图文视频(txt: str) -> bool:
    """
    判断输入文本是否包含图文视频的核心特征（图片+数字+中文逗号+按钮）

    参数:
        txt: 待检测的文本字符串

    返回:
        bool: 包含图文特征返回True，否则返回False

    Doctest单元测试用例：
    >>> 是否为图文视频('关注 私信，按钮 赤壁付 未点赞，喜欢29，按钮 29 评论17，按钮 评论17，按钮 17 未选中，收藏1，按钮 1 分享，按钮 分享 音乐，@流浪的魂斗罗创作的原声，按钮 互相关注 咸宁市 | 玄素洞 @赤壁付 · 15分钟前 发布时间：15分钟前 【🌺小寒日·花间暖舟】载一船冬日的诗意停泊你眼前。... 展开 图片1，按钮 图片2，按钮 图片3，按钮 图片4，按钮 图片5，按钮 图片6，按钮 图片7，按钮 图片8，按钮 图片9，按钮 图片10，按钮 暂停视频，按钮')
    True
    >>> 是否为图文视频('纯视频内容，没有图片相关文字，暂停视频，按钮')
    False
    >>> 是否为图文视频('图片0，按钮 这是包含单数字的图文内容')
    True
    >>> 是否为图文视频('图片100，按钮 这是包含多数字的图文内容')
    True
    >>> 是否为图文视频('图片，按钮 没有数字的情况（非图文）')
    False
    >>> 是否为图文视频('')
    False
    >>> 是否为图文视频('图片abc，按钮 非数字字符（非图文）')
    False
    """
    # 正则表达式匹配核心特征：图片+1个及以上数字+中文逗号+按钮
    # \d+ 匹配1个或多个数字，确保是"图片+数字"的格式
    pattern = r"图片\d+，按钮"

    # 使用search查找是否存在匹配内容（找到1个即可判定）
    match_result = re.search(pattern, txt)

    # 有匹配结果返回True，无则返回False
    return match_result is not None


def 提取点赞与评论数(txt: str) -> tuple[int, int]:
    """
    从混合文本中提取点赞数（喜欢后接的数字）和评论数（评论后接的数字）

    单元测试示例：
    >>> 提取点赞与评论数("关注 私信，按钮 大漠雄鹰 未点赞，喜欢597，按钮 597 评论447，按钮 评论447，按钮 447 未选中，收藏收藏")
    (597, 447)
    """
    # 正则匹配"喜欢"后紧跟的数字（捕获组提取数字部分）
    like_match = re.search(r"喜欢(\d+)，按钮", txt)
    点赞数 = int(like_match.group(1)) if like_match else 0

    # 正则匹配"评论"后紧跟的数字（捕获组提取数字部分）
    comment_match = re.search(r"评论(\d+)，按钮", txt)
    评论数 = int(comment_match.group(1)) if comment_match else 0

    return 点赞数, 评论数


def has_interaction_keywords(text):
    """
    检测输入文本中是否包含指定的互动类关键词中的任意一个。

    指定关键词列表：串门 互粉 窜门 互助 互关 互赞 互访 回关
    匹配规则：严格匹配完整关键词（区分全角/半角、空格分隔等），只要包含任意一个即返回True。

    参数:
        text (str): 需要检测的文本字符串（空字符串会返回False）

    返回:
        bool: 包含任意一个关键词返回True，否则返回False

    示例:
    >>> has_interaction_keywords("我来串门了，有空一起聊")  # 包含"串门"
    True
    >>> has_interaction_keywords("互粉互关，一起涨粉")  # 包含"互粉"+"互关"
    True
    >>> has_interaction_keywords("窜门的朋友请留步")  # 包含"窜门"（注意和"串门"的区别）
    True
    >>> has_interaction_keywords("大家互相帮助，互助共赢")  # 包含"互助"
    True
    >>> has_interaction_keywords("点赞后记得回关哦")  # 包含"回关"
    True
    >>> has_interaction_keywords("互赞一下，谢谢啦！")  # 包含"互赞"（带标点）
    True
    >>> has_interaction_keywords("有空互相访问，互访增加活跃度")  # 包含"互访"
    True
    >>> has_interaction_keywords("今天天气不错，适合出门散步")  # 无任何关键词
    False
    >>> has_interaction_keywords("")  # 空文本
    False
    >>> has_interaction_keywords("关键词是互粉吗？是的")  # 关键词在句子中间
    True
    >>> has_interaction_keywords("回关")  # 仅包含单个关键词
    True
    >>> has_interaction_keywords("记得互助！")  # 关键词带感叹号
    True
    >>> has_interaction_keywords("我想要互访，你愿意吗？")  # 关键词在句子中
    True
    >>> has_interaction_keywords("串门和窜门都算吗？算的")  # 包含两个相似关键词
    True
    >>> has_interaction_keywords("互 粉 这样分开的不算")  # 关键词被空格分隔，不匹配
    False
    >>> has_interaction_keywords("互芬（错别字）不算")  # 关键词错别字，不匹配
    False
    >>> has_interaction_keywords("回 关")  # 关键词拆分，不匹配
    False
    >>> has_interaction_keywords('、伙伴计划 发布者：@墨墨妈(有关必回)，配文“每天进步一点点，终会成功!姐妹们一起加油!”')
    True
    """
    # 定义需要检测的核心关键词列表（严格按用户要求）
    interaction_keywords = [
        "串门",
        "互粉",
        "窜门",
        "互助",
        "互关",
        "互赞",
        "互访",
        "回关",
        "有关必回",
        "伙伴计划",
    ]

    # 遍历关键词列表，检查是否有任意一个出现在文本中
    # 一旦匹配到立即返回True，提升执行效率
    for keyword in interaction_keywords:
        if keyword in text:
            return True

    # 未匹配到任何关键词，返回False
    return False


def 从文本中提取用户名称(txt: str) -> str:
    """
    从文本字符串中提取目标用户名，提取不到时返回None

    测试用例：
    >>> txt1 = '''关注 私信，按钮 森森火山泥工厂 未点赞，喜欢3，按钮 3 评论3，按钮 评论3，按钮 3 未选中，收藏收藏，按钮 收藏 分享，按钮 分享 音乐，来自模板的原声，按钮 临沂市 | 三立大厦 @森森火山泥工厂 互相串门了 > 暂停视频，按钮 提取的图片相关详细信息如下： 发布时间：上午11:51​核心内容：“今天继续串门 流量增长真实有效 森森火山 各位兄弟姐妹都在家”“临沂市|三立大厦 @森森火山泥工厂 互相串门了”​互动数据：3个点赞、3个收藏​相关入口：关注、商城、推荐、首页、朋友、消息​操作功能：分享、收藏 需要我帮你生成类似的串门引流文案，适配社交平台发布吗？'''
    >>> 从文本中提取用户名称(txt1)
    '森森火山泥工厂'

    >>> txt2 = '''关注 @张张要努力（涨粉中） 未点赞，喜欢17，按钮 17 评论20，按钮 评论20，按钮 20 未选中，收藏收藏，按钮 收藏 分享，按钮 分享 音乐，@小小鹿创作的原声，按钮 @张张要努力（涨粉中） #DOU #热门 #串门儿 #流量 > 暂停视频，按钮 从提供的网页内容提取到该链接图片相关的详细信息如下： 时间：半夜12:27​关联标签：#DOU#、#热门#、#串门儿#、#流量#​发布账号：@张张要努力(涨粉中)​相关功能入口：首页、消息、我、收藏、分享、关注、商城推荐、点击激活​数字信息：100、17、20（具体含义未明确，推测可能为点赞、浏览相关数据）'''
    >>> 从文本中提取用户名称(txt2)
    '张张要努力'

    >>> txt3 = '''关注 🍀晓宁 未点赞，喜欢309，按钮 309 评论364，按钮 评论364，按钮 364 未选中，收藏3，按钮 3 分享，按钮 分享 音乐，@穿越时空创作的原声，按钮 @🍀晓宁 帮我算一下得等多久才能到五千粉 #真实生活分享官 展开 图片1，按钮 暂停视频，按钮 核心信息 相关计划：抖音/抖音精选APP的创作者伙伴计划，加入后发布作品可按内容质量、获利播放、有效互动、用户价值计算收益（普通作品得基础收益，精选作品得基础+精选收益）。​加入条件：抖音账号互动粉丝数≥5000（当前110，未满足）、2022年起公开作品数>20（当前364，已满足）。​用户诉求：当前每天仅增加1个互动粉丝，疑惑需多久达到5000粉。​其他内容：平台提供专题课程（抖音优质视频创作指南、抖音热门玩法教程合集），账号收到3条消息。 需要我帮你估算按当前涨粉速度达成5000粉的具体时间，或整理涨粉提速的实用建议吗？'''
    >>> 从文本中提取用户名称(txt3)
    '🍀晓宁'

    >>> txt4 = '''关注 @有个朋友🍋 未点赞，喜欢6，按钮 6 评论3，按钮 评论3，按钮 3 未选中，收藏收藏，按钮 收藏 分享，按钮 分享 拍同款 音乐，@NiNi高品鞋创作的原声，按钮 @@有个朋友🍋 来了，来了，串门来了 #抖音伙伴计划来了 #创作者扶持... 展开 图片1，按钮 暂停视频，按钮 核心信息 页面场景：抖音APP内相关功能页面，包含创作扶持、伙伴计划等内容​关键模块：​抖音伙伴计划：加入条件为年龄≥18周岁、抖音账号互动粉丝数5000（显示“已满足”“未满足”状态）​创作资源：抖音优质视频创作指南、抖音热门玩法教程合集​其他内容：用户动态（“来了，来了，我要来串门了，今天目标300+”）、相关话题#抖音伙伴计划来了#创作者扶持...​反馈渠道：前往「抖音app-我-我的客服」反馈问题​页面功能入口：收藏、分享、发作品、拍同款，底部有首页、消息、我等导航栏 需要我帮你整理成清晰的表格形式，方便快速查看关键信息吗？'''
    >>> 从文本中提取用户名称(txt4)
    '有个朋友🍋'

    >>> txt5 = '这是一段没有有效用户名的文本，只有@小小鹿创作的原声，无目标用户'
    >>> 从文本中提取用户名称(txt5) is None
    True

    >>> txt1 = '''关注 私信，按钮 森森火山泥工厂 未点赞，喜欢3，按钮 3 评论3，按钮 评论3，按钮 3 未选中，收藏收藏，按钮 收藏 分享，按钮 分享 音乐，来自模板的原声，按钮 临沂市 | 三立大厦 @森森火山泥工厂 互相串门了 > 暂停视频，按钮 提取的图片相关详细信息如下： 发布时间：上午11:51​核心内容：“今天继续串门 流量增长真实有效 森森火山 各位兄弟姐妹都在家”“临沂市|三立大厦 @森森火山泥工厂 互相串门了”​互动数据：3个点赞、3个收藏​相关入口：关注、商城、推荐、首页、朋友、消息​操作功能：分享、收藏 需要我帮你生成类似的串门引流文案，适配社交平台发布吗？'''
    >>> 从文本中提取用户名称(txt1)
    '森森火山泥工厂'

    >>> txt2 = '''关注 @张张要努力（涨粉中） 未点赞，喜欢17，按钮 17 评论20，按钮 评论20，按钮 20 未选中，收藏收藏，按钮 收藏 分享，按钮 分享 音乐，@小小鹿创作的原声，按钮 @张张要努力（涨粉中） #DOU #热门 #串门儿 #流量 > 暂停视频，按钮 从提供的网页内容提取到该链接图片相关的详细信息如下： 时间：半夜12:27​关联标签：#DOU#、#热门#、#串门儿#、#流量#​发布账号：@张张要努力(涨粉中)​相关功能入口：首页、消息、我、收藏、分享、关注、商城推荐、点击激活​数字信息：100、17、20（具体含义未明确，推测可能为点赞、浏览相关数据）'''
    >>> 从文本中提取用户名称(txt2)
    '张张要努力'

    >>> txt3 = '''关注 🍀晓宁 未点赞，喜欢309，按钮 309 评论364，按钮 评论364，按钮 364 未选中，收藏3，按钮 3 分享，按钮 分享 音乐，@穿越时空创作的原声，按钮 @🍀晓宁 帮我算一下得等多久才能到五千粉 #真实生活分享官 展开 图片1，按钮 暂停视频，按钮 核心信息 相关计划：抖音/抖音精选APP的创作者伙伴计划，加入后发布作品可按内容质量、获利播放、有效互动、用户价值计算收益（普通作品得基础收益，精选作品得基础+精选收益）。​加入条件：抖音账号互动粉丝数≥5000（当前110，未满足）、2022年起公开作品数>20（当前364，已满足）。​用户诉求：当前每天仅增加1个互动粉丝，疑惑需多久达到5000粉。​其他内容：平台提供专题课程（抖音优质视频创作指南、抖音热门玩法教程合集），账号收到3条消息。 需要我帮你估算按当前涨粉速度达成5000粉的具体时间，或整理涨粉提速的实用建议吗？'''
    >>> 从文本中提取用户名称(txt3)
    '🍀晓宁'

    >>> txt4 = '''关注 @有个朋友🍋 未点赞，喜欢6，按钮 6 评论3，按钮 评论3，按钮 3 未选中，收藏收藏，按钮 收藏 分享，按钮 分享 拍同款 音乐，@NiNi高品鞋创作的原声，按钮 @@有个朋友🍋 来了，来了，串门来了 #抖音伙伴计划来了 #创作者扶持... 展开 图片1，按钮 暂停视频，按钮 核心信息 页面场景：抖音APP内相关功能页面，包含创作扶持、伙伴计划等内容​关键模块：​抖音伙伴计划：加入条件为年龄≥18周岁、抖音账号互动粉丝数5000（显示“已满足”“未满足”状态）​创作资源：抖音优质视频创作指南、抖音热门玩法教程合集​其他内容：用户动态（“来了，来了，我要来串门了，今天目标300+”）、相关话题#抖音伙伴计划来了#创作者扶持...​反馈渠道：前往「抖音app-我-我的客服」反馈问题​页面功能入口：收藏、分享、发作品、拍同款，底部有首页、消息、我等导航栏 需要我帮你整理成清晰的表格形式，方便快速查看关键信息吗？'''
    >>> 从文本中提取用户名称(txt4)
    '有个朋友🍋'

    >>> txt5 = '这是一段没有有效用户名的文本，只有@小小鹿创作的原声，无目标用户'
    >>> 从文本中提取用户名称(txt5) is None
    True

    # 专项测试：全角括号单独场景
    >>> txt6 = '@测试用户（全角括号内容） 测试文本 @测试用户（再次出现）'
    >>> 从文本中提取用户名称(txt6)
    '测试用户'

    # 专项测试：混合全角/半角括号
    >>> txt7 = '@混合括号(半角)测试（全角）123 @混合括号(半角)测试（全角）456'
    >>> 从文本中提取用户名称(txt7)
    '混合括号测试123'


    >>> txt8 = '关注 凯利童车 未点赞，喜欢159，按钮 159 评论134，按钮 评论134，按钮 134 未选中，收藏1，按钮 1 分享，按钮 分享 音乐，@幸运小锦鲤创作的原声，按钮 郑州市 | 凯利童车 @凯利童车 串门啦…… > 暂停视频，按钮时间：上午7:47 相关账号：郑州市|凯利童车（@凯利童车）内容：“我来串门啦......今天串 5000家。”​互动数据：159次分享、134条相关互动（具体类型未明确）​页面元素：包含“首页”“朋友”“消息”“我”等导航栏选项，以及“团购经验”“成都”“关注”“商城”“推荐”等标签'
    >>> 从文本中提取用户名称(txt8)
    '凯利童车'

    """

    # 1. 正则匹配所有@后的候选用户名（仅排除@/#/>/空白字符，保留括号方便后续统一清理）
    # 适配单个@/多个@（如@@有个朋友🍋），\s匹配所有空白字符（空格、制表符等）
    candidate_pattern = r"@+([^@#> \s]+)"
    all_candidates = re.findall(candidate_pattern, txt)

    # 2. 过滤音乐创作者相关的无效候选（排除"创作的原声"等干扰项）
    filter_keywords = ["创作的原声", "创作的"]
    valid_candidates = [
        cand
        for cand in all_candidates
        if not any(keyword in cand for keyword in filter_keywords)
    ]

    # 无有效候选时直接返回None
    if not valid_candidates:
        return None

    # 3. 统计候选出现次数，取出现最频繁的（目标用户会多次出现）
    cand_counter = Counter(valid_candidates)
    most_frequent_cand = cand_counter.most_common(1)[0][0]

    # 4. 核心：清理所有全角/半角括号及括号内内容（重点强化全角括号匹配）
    # 正则说明：
    # [（\(] → 匹配全角左括号（ 或半角左括号(
    # [^）\)]* → 匹配括号内任意内容（非全角右括号）、非半角右括号)的所有字符
    # [）\)] → 匹配全角右括号） 或半角右括号)
    clean_pattern = r"[（\(][^）\)]*[）\)]"
    # 循环清理：确保嵌套括号、多次括号都能被彻底移除
    clean_username = most_frequent_cand
    while re.search(clean_pattern, clean_username):
        clean_username = re.sub(clean_pattern, "", clean_username).strip()

    # 极端情况：清理后为空则返回None
    return clean_username if clean_username else None


def remove_action_markers(text: str) -> str:
    """
    仅移除文本**开头部分**被方括号 [] 包裹的动作标记（如 [发了一个表情包]、[比心] 等）
    文本中间/结尾的动作标记会保留不变

    参数:
        text: 包含动作标记的原始文本

    返回:
        仅移除开头动作标记后的文本（首尾多余空格会清理）

    示例:
    >>> remove_action_markers("[发了一个表情包]比心回应你！看起来你心情不错")
    '比心回应你！看起来你心情不错'

    # 仅移除开头标记，中间/结尾的标记保留
    >>> remove_action_markers("[发了一个表情包]你好[比心]啊[大笑]")
    '你好[比心]啊[大笑]'

    # 开头无标记，返回原文本（仅清理首尾空格）
    >>> remove_action_markers("看起来你心情不错[比心]，是不是有好消息？")
    '看起来你心情不错[比心]，是不是有好消息？'

    # 空字符串输入
    >>> remove_action_markers("")

    # 开头多个连续标记，全部移除
    >>> remove_action_markers("[动作1][动作2]开头文本[动作3]")
    '[动作2]开头文本[动作3]'

    # 标记在结尾，不处理
    >>> remove_action_markers("结尾文本[动作]")
    '结尾文本[动作]'

    # 开头无标记、中间有标记，完全保留
    >>> remove_action_markers("中间[动作]部分")
    '中间[动作]部分'

    # 开头标记含特殊字符，正常移除
    >>> remove_action_markers("[发了😀表情包]文本[比心❤️]内容")
    '文本[比心❤️]内容'
    >>> remove_action_markers(None)
    >>> remove_action_markers('')
    """
    if text is None or not text.strip():
        return
    # 正则表达式：^ 匹配文本开头，\[.*?\] 匹配方括号包裹的标记
    # count=1 确保只替换一次（即使开头有多个连续标记，也能通过一次匹配覆盖）
    pattern = r"^\[.*?\]"
    # 替换开头的动作标记为空字符串，strip()清理替换后可能的首尾空格
    cleaned_text = re.sub(pattern, "", text, count=1)
    return cleaned_text.strip()


def 去掉最后一句问句(txt: str) -> str:
    """
    去掉文本中最后的一个问句，返回剩余的部分。

    测试示例（与需求一致）：
    >>> txt = '''
    ... 对方的最后一条消息旁显示“已读”状态，表明对方已经看到了你“没问题，你说~”的回复。
    ...
    ... 整体氛围
    ...
    ... 这段对话充满了同频感，两位创作者在互相鼓励、抱团取暖，围绕抖音做号的“观望期”“流量迷局”等共同话题展开，传递出轻松、互助的友好氛围。
    ...
    ... 要不要我帮你准备几个高情商的回复模板，让你和这类同频创作者的聊天更顺畅？'''
    >>> result = 去掉最后一句问句(txt)
    >>> expected = '''对方的最后一条消息旁显示“已读”状态，表明对方已经看到了你“没问题，你说~”的回复。
    ... 整体氛围
    ... 这段对话充满了同频感，两位创作者在互相鼓励、抱团取暖，围绕抖音做号的“观望期”“流量迷局”等共同话题展开，传递出轻松、互助的友好氛围。'''
    >>> result.strip() == expected.strip()  # 忽略首尾空白对比
    True

    # 测试边界场景1：无问句的文本
    >>> 去掉最后一句问句("这是一段普通的文本，没有问句。")
    '这是一段普通的文本，没有问句。'

    # 测试边界场景2：文本只有一个问句
    >>> 去掉最后一句问句("你好吗？")
    ''

    # 测试边界场景3：问句不在最后一行（不处理）
    >>> 去掉最后一句问句("今天天气好吗？是的，很好。")
    '今天天气好吗？是的，很好。'

    # 测试边界场景4：多个问句，只移除最后一个
    >>> 去掉最后一句问句("吃饭了吗？喝水了吗？今天开心吗？")
    ''
    >>> 去掉最后一句问句(None)
    """
    # 1. 将文本按换行符拆分成行列表，保留原格式结构
    if txt is not None:
        lines = txt.split("\n")

        lines = [line.strip() for line in lines if line.strip()]

        if lines and lines[-1].endswith("？"):
            lines = lines[:-1]
        return "\n".join(lines)


# 运行doctest单元测试
if __name__ == "__main__":
    import doctest

    print(doctest.testmod(verbose=False, report=False))
