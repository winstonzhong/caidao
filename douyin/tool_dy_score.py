...
def 检测文本包含词(文本: str, 词列表: list) -> bool:
    """
    辅助函数：检测文本中是否包含词列表中的任意一个词（正则匹配，自动转义特殊字符）

    >>> 检测文本包含词("这是测试视频，内容很好", ["测试", "内容"])
    True
    >>> 检测文本包含词("这是测试视频", ["违禁词1", "违禁词2"])
    False
    >>> 检测文本包含词("", ["测试"])
    False
    >>> 检测文本包含词("包含特殊字符.？*", [".？*"])  # 验证特殊字符转义有效性
    True
    """
    # 空文本或空词列表直接返回False
    if not 词列表 or not 文本:
        return False
    # 对每个词进行正则转义，避免特殊字符（如.、*）干扰匹配
    转义后的词 = [re.escape(词) for 词 in 词列表]
    # 构建匹配任意一个词的正则表达式
    匹配模式 = re.compile("|".join(转义后的词))
    # 搜索文本中是否存在匹配项
    return bool(匹配模式.search(文本))
...
def 计算视频价值评分(
    ...
    """
    核心函数：根据视频文本、互动数据、违禁词/关键词等条件计算视频价值评分
    ...
    测试用例（固定随机种子保证可复现）：
    >>> # 测试1：包含违禁词，返回0
    >>> 计算视频价值评分("这是测试视频, 有违禁词.", 10, 20, 5, 3, "违禁词", "测试", 1000, 1, 1)
    0.0
    >>> # 测试2：关键词非空但文本不含，返回0
    >>> 计算视频价值评分("这是普通视频", 10, 20, 5, 3, "", "测试", 1000, 1, 1)
    0.0
    >>> # 测试3：点赞数超上限，返回0
    >>> 计算视频价值评分("这是测试视频", 10, 2000, 5, 3, "", "测试", 1000, 1, 1)
    0.0
    >>> 计算视频价值评分("这是测试视频", 10, 20, 5, 3, "", "测试", 1000, 1, 30)
    100.0
    >>> 计算视频价值评分("这是测试视频", 10, 20, 5, 3, "", "测试", 1000, 1, 50)
    0.0
    >>> # 测试6：关键词为空，其他条件满足，返回100
    >>> 计算视频价值评分("任意文本", 5, 5, 5, 5, "", "", 1000, 1, 50)
    0.0

    >>> 计算视频价值评分("这是测试视频", 10, 20, 5, 3, "", "测试", 10, 1, 6)
    0.0
    """
    # 1. 处理违禁词/关键词拆分（去除空字符串和空格干扰）
    违禁词列表 = 违禁词.strip().split(",") if 违禁词.strip() else []
    关键词列表 = 关键词.strip().split(",") if 关键词.strip() else []

    # 2. 处理随机数：0则生成[下限,上限]的随机数，否则用传入值
    if 最低互动总数 == 0:
        最终随机数 = random.randint(下限, 上限)
        print(f"=================随机数：{最终随机数}")
    else:
        最终随机数 = 最低互动总数
    # 最终随机数 = random.randint(下限, 上限) if 最低互动总数 == 0 else 最低互动总数

    # 3. 检查互动数据是否超上限（任意一个超上限则返回0）
    互动数据 = [评论数, 点赞数, 转发, 收藏]
    if any(数据 > 上限 for 数据 in 互动数据):
        return 0.0

    # 4. 检查违禁词：包含则返回0
    if 检测文本包含词(视频文本, 违禁词列表):
        return 0.0

    # 5. 检查关键词：关键词非空且文本不含则返回0
    if 关键词列表 and not 检测文本包含词(视频文本, 关键词列表):
        return 0.0

    # 6. 检查互动总和与随机数的关系：总和≥随机数则返回0
    互动总和 = sum(互动数据)
    if 互动总和 < 最终随机数:
        return 0.0
...
def 计算评论价值评分(
    ...
    if 随机数 == 0:
        随机数 = random.randint(200, 1000)
        print(f"--------------随机数:{随机数}----------------------")

    # 第四步：判断评论数/点赞数是否超过随机数
    if 点赞数 + 评论数 + 转发 + 收藏 < 随机数:
        print(
            f"!!!!!!!!!!!!!!!!!!!!!热度不足:{点赞数+评论数+转发+收藏} < {随机数} !!!!!!!!!!!!!!!!!"
        )
        return 0.0
