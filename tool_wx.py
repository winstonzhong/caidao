...
def is_not_equal_ignore_null(d1, d2, name):
    ...
    """
    if (
        pandas.isnull(d1.get(name))
        or pandas.isnull(d2.get(name))
        or d1.get(name) == d2.get(name)
    ):
        return False
...
def is_up_down_same(d1, d2):
    ...
    if (d1.get("upside") and d2.get("underside")) or (
        d2.get("upside") and d1.get("underside")
    ):
        return True
