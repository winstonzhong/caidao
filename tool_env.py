...
def 精确识别并截断废话(txt: str) -> str:
    ...
    >>> 精确识别并截断废话("晚上好呀～ 每天收到你的问候都超开心！比心回赠，愿你也被温柔和好运包围，元气满满一整天呀～")
...
def truncate_at_last_punct_if_question(text: str) -> str:
    ...
    'Hi～ 刚看到你的问候呀！'
    >>> truncate_at_last_punct_if_question("随便聊聊伙伴计划创作～ 🌟🍀→小小多！之前拍这类内容，投稿数比之前涨了一倍多，完播率也有二成出头，慢慢摸索着确实在变好，一起加油呀～ 需要我针对其他视频主题，按同样风格生成新的评论吗？")
    '随便聊聊伙伴计划创作～ 🌟🍀→小小多！之前拍这类内容，投稿数比之前涨了一倍多，完播率也有二成出头，慢慢摸索着确实在变好，一起加油呀～'
    """
...
def 截断问句和废话(txt):
    ...
    '串门氛围好轻松呀！主动去串门互动还挺能拉近距离的～我之前想串门总有点不好意思开口，你平时串门的时候，一般会先从哪些话题聊起，来让互动更自然呀～'
...
def 对豆包回复进行所有的必要处理(txt):
    ...
    #>>> 对豆包回复进行所有的必要处理("哇这广场也太开阔了吧！青山配高楼的景色好舒服，为500家的目标加油呀～")
    """
    txt = 截断问句和废话(txt)
