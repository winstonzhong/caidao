...
from adb_tools.common.exceptions import (
    ElementNotFoundError,
    NotNeedFurtherActions,
    TplNotFoundError,
)
...
def scroll_to_find(
    adb, xpath, text, fun_scroll=scroll_to_left, fun_match=is_element_wanted
):
...
class BaseAdb(object):
    ...
    def is_in_overview(self):
        xpath = (
            '//android.widget.FrameLayout[re:match(@resource-id, ".*/task_view")]',
        )
    ...
    def icon(self):
        return (
            cut_empty_margin(rgb_to_mono(cv2.imread(str(self.fpath_icon))))
            if self.fpath_icon.exists()
            else None
        )
    ...
    def fix_jy_update(self):
        e1 = self.find_xpath_safe(
            '//android.widget.Button[re:match(@resource-id, ".*/btn_update_cancel")][@text="取消"]'
        )
        e2 = self.find_xpath_safe(
            '//android.widget.Button[re:match(@resource-id, ".*/btn_update_sure")][@text="更新"]'
        )
        if e1.exists and e2.exists:
            e1.click()
            return True
    ...
    def fix_jy_no_in_jianji_tab(self):
        e1 = self.find_xpath_safe(
            '//android.widget.TextView[re:match(@resource-id, ".*/tab_name")][@text="剪辑"]'
        )
        if e1.exists:
            e1.click()
            return True
    ...
    def fix_db_update(self):
        ...
        e1 = self.find_xpath_safe(
            '//android.widget.TextView[re:match(@resource-id, ".*/tvDialogCancel")][@text="忽略"]'
        )
        e2 = self.find_xpath_safe(
            '//android.widget.TextView[re:match(@resource-id, ".*/btnDialogConfirm")][@text="立即更新"]'
        )
        if e1.exists and e2.exists:
            e1.click()
            return True
    ...
    def fix_jy_come_gao(self):
        ...
        e1 = self.find_xpath_safe(
            '//android.widget.ImageView[re:match(@resource-id, ".*/ivClose")][@text=""]'
        )
        e2 = self.find_xpath_safe(
            '//android.widget.Button[re:match(@resource-id, ".*/btnConfirm")]'
        )
        if e1.exists and e2.exists:
            e1.click()
            return True
    ...
    def get_files(self, base_dir="/sdcard/DCIM/Camera", ptn="*"):
        ...
        if r.exit_code == 0 and r.output:
            return list(
                map(lambda x: self.remote_join(base_dir, x), r.output.splitlines())
            )
        return []
    ...
    def pull_lastest_file_to_local_tmp(
        self, base_dir="/sdcard/DCIM/Camera", tmp_dir=None
    ):
        return self.pull_lastest_file(
            to_dir=TMP_DIR if tmp_dir is None else tmp_dir, base_dir=base_dir
        )
    ...
    def push_file_to_pictures(self, src, sleep_span=0.1, use_timestamp=True):
        return self.push_file_to_temp(
            src, sleep_span, False, use_timestamp, self.PICTURES_DIR
        )

    def push_file_to_download(
        self,
        src,
        sleep_span=0.1,
        clean_temp=False,
    ...
    def open_app(self, force_open=False, use_monkey=False):
        if force_open or not self.is_app_opened():
            self.ua2.app_start(
                self.app_name, activity=self.activity, stop=True, use_monkey=use_monkey
            )
        return self
    ...
    def open_filemanager(self):
        d = {
            "package_name": "com.huawei.filemanager",
    ...
    def open_filemanager2(self):
        d = {
            "package_name": "com.hihonor.filemanager",
    ...
    def start_scrcpy(cls, ip_port, encoding="utf8", shell=True, return_directly=False):
        process = subprocess.Popen(
            f"scrcpy -s {ip_port} --no-audio --always-on-top",
            encoding=encoding,
            shell=shell,
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
    ...
    def setup(self, encoding="utf8"):
        process = subprocess.Popen(
            f'{adb_exe} -s {self.device["id"]} tcpip {self.device["port"]}',
            encoding=encoding,
            shell=True,
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
        ...
        process = subprocess.Popen(
            f"{adb_exe} connect {self.ip_port}",
            encoding=encoding,
            shell=True,
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
    ...
    def test(self):
        print(
            self.execute(
                "am start-activity -a com.android.settings/.Settings\$LocationSettingsActivity"
            )
        )
    ...
    def first_device_name(cls, name):
        return list(filter(lambda x: x.get("name") == name, cls.get_devices_as_dict()))[
            0
        ]
    ...
    def first_device_model(cls, name):
        return list(
            filter(lambda x: x.get("model") == name, cls.get_devices_as_dict())
        )[0]
    ...
    def move_flac_to_pc(self):
        self.move_files_to_pc(
            cmd='find / -name *.flac 2>&1 | grep -v "Permission denied"'
        )
    ...
    def screen_shot(self):
        b = self.execute(script=b"screencap -p", encoding=None)[0]
        if OS_WIN:
            b = b.replace(b"\r\n", b"\n")
        return bin2img(b)
    ...
    def 特殊双击(self, x, y):
        self.execute(
            f"input tap {x} {y}&sleep 0.1;input tap {x} {y}&sleep 0.01;input tap {x} {y}"
        )
        return self
        # self.do_click(x, y)
        # time.sleep(0.1)
    ...
    def switch_app_steady_open_if_not_found(
        self,
        name,
    ):
        pass
