    ...
    def 上传到下载目录(self, url, fname=None, clean_temp=True):
        ...
        if tool_static.is_inner() and tool_static.is_56t_url(url):
            ...
            if fname:
                ...
                # 如果 basename 为空或没有后缀，使用 timestamp + .jpg
                if not filename or '.' not in filename:
                    filename = f"{time.time()}.jpg"
            
            src = f"/sdcard/Download/{filename}"
...
class 基本任务(抽象持久序列):
    ...
    def _提取配置覆盖值字典(self, 配置):
        ...
        支持三种配置格式：
        1. 旧格式: {"目标视频描述": "...", ...}
        2. 前端格式: {"keys": [{"name": "...", "current_value": "..."}]}
        3. 数据库存储格式: {"目标视频描述": "...", "目标视频描述_历史": [...], "视频评论提示词_pdd_rights": "...", ...}
           - 无下划线的 key → 普通配置项
           - 以 _历史/_描述 结尾 → 元数据，忽略
           - 其他带下划线的 → 内置配置项，忽略
        ...
        # 【修改】数据库存储格式：不再过滤，返回全部配置
        # 包括：普通配置项、元数据（_历史/_描述）、内置配置项（带下划线）
        # 过滤只在 views 层做（前端展示时）
        return dict(配置)
